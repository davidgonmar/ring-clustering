...
def random_circle(base_position: np.ndarray, radius: float, n_samples: int, noise: float = 0.1):
...
def random_circles(center_delimiters, min_max_radius, n_samples_per_circle, n_rings, noise=0.3):
    assert center_delimiters.shape == (4, 2), "center_delimiters must be a 4x2 array defining the corners of the rectangle"
    data = []
    ...
    centers = np.array([[np.random.uniform(min_x, max_x), np.random.uniform(min_y, max_y)] for _ in range(n_rings)])
    print(centers)
